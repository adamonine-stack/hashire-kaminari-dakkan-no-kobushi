...
func run() -> void:
	...
	check(hud.enemy_icon_rect.texture != null, "HUD switches enemy face icon to Rei")
	check(hud.player_name_label.visible and hud.player_name_label.text == "ゴウ", "HUD shows selected ally name in katakana")
	check(hud.player_icon_rect.texture != null, "HUD shows selected ally face icon")
