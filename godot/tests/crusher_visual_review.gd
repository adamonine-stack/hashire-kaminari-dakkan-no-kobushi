...
# godot --path godot --script res://tests/crusher_visual_review.gd
...
func review() -> void:
    if DisplayServer.get_name() == "headless":
        push_error("Crusher frame review needs an actual rendering display.")
        quit(1)
        return
    var output := ProjectSettings.globalize_path("res://../audit_evidence/crusher_visual_review")
    ...
    if not manager.isRoundActive:
        push_error("Could not reach Stage 1 for Crusher frame review.")
        quit(1)
        return
    ...
    for clip in atlas.clips:
        if not sprite.sprite_frames.has_animation(clip):
            push_error("Missing Crusher animation: " + clip)
            quit(1)
            return
    ...
    for side in [-1, 1]:
        ...
        for action in ["Punch", "Kick"]:
            ...
            enemy.finish_attack()
    print("CRUSHER_VISUAL_REVIEW_OK clips=", atlas.clips.size(), " output=", output)
